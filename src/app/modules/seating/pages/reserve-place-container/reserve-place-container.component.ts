...
export class ReservePlaceContainerComponent implements OnInit, AfterViewInit {
  ...
  processSvgImage() {
    if (this.selectedSvgImage) {
      this.desks =
        this.selectedSvgImage.querySelectorAll('#Bookable_Slots')[0].childNodes;
      ...
      this.desks.forEach((svgElement: any, index: any) => {
        ...
        if (bookingPlace.state == ChairTypeEnum.free) {
          ...
          bookingPlace.state == ChairTypeEnum.fixed ||
          bookingPlace.state == ChairTypeEnum.reserved
        ) {
  ...
  processParkingSvgImage(elementRef: any) {
    if (this.selectedSvgImage) {
      ...
      this.desks.forEach((svgElement: any, index: any) => {
        ...
        svgElement.addEventListener('click', () => {
          this._store.dispatch(OpenBookDeskModal({ payload: 1 }));
        });
